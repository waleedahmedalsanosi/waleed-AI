...
2. Get the meeting transcript or notes from the user — same as `/intake`:
   - If the user provides a file path (starts with `~/` or `/`), read that file.
   - If the user pastes text, use it directly.
3. Read any previous meeting notes in `{SESSION}/meetings/` for continuity.

### Step 2: Write the meeting note
