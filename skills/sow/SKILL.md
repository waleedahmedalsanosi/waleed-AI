...
if [ ! -f "$SESSION/04-prd.md" ]; then
  echo "ERROR: 04-prd.md not found — run /prd first"; exit 1
fi
FIRST_LINE=$(head -1 "$SESSION/04-prd.md" 2>/dev/null)
if [ "$FIRST_LINE" != "# Product Requirements Document" ]; then
  echo "ERROR: 04-prd.md is empty or malformed — re-run /prd"; exit 1
fi
...
- `04-prd.md` — the Product Requirements Document (scope, features, timeline)
...
### Step 2: Write 05-sow.md

Write to `{SESSION}/05-sow.md`:
